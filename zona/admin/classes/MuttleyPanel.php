 ...
 * @author          Mariusz Rek
 * @copyright       2017 Muttley Framework
 */
	...
	class MuttleyPanel {
        ...
        */
		function admin_bar() {
			...
			$wp_admin_bar->add_menu(
				array( 
					'id' => 'r_theme_settings', 
					'title' => esc_html__( 'Theme Options', 'zona' ), 
        ...
        */
		public function muttleypanel_enqueue() {
			...
			if ( $current_screen->base === $current_page  ) {
				...
				/* Nprogress */
				wp_enqueue_style( 'nprogress_css', $this->args['admin_path'] . '/assets/css/nprogress.min.css' );
				wp_enqueue_script( 'nprogress_js', $this->args['admin_path'] . '/assets/js/nprogress.min.js', false, false, true );

				/* Notify */
				wp_enqueue_script( 'notify_js', $this->args['admin_path'] . '/assets/js/jquery.notify.min.js', false, false, true);

				/* UI */
				wp_enqueue_style( 'MuttleyPanel_ui_css', $this->args['admin_path'] . '/assets/css/jquery-ui.min.css' );

				/* Panel stylesheet */
				wp_enqueue_style( 'MuttleyPanel_css', $this->args['admin_path'] . '/assets/css/panel.min.css' );

				/* Panel fonts */
				wp_enqueue_style( 'MuttleyPanel_font', $this->args['admin_path'] . '/assets/css/font-awesome.min.css' );
				
				/* Panel javascripts */
				wp_enqueue_script( 'MuttleyPanel_core', $this->args['admin_path'] . '/assets/js/core.min.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-dialog', 'jquery-ui-widget', 'jquery-ui-sortable', 'jquery-ui-droppable', 'jquery-ui-slider', 'jquery-ui-draggable', 'jquery-ui-datepicker'), false, true );

				/* Panel Fields */
				wp_enqueue_script( 'MuttleyPanel_fields', $this->args['admin_path'] . '/assets/js/fields.min.js', false, false, true);
				$js_variables = array(
        ...
        */
		function display() {
	        ...
	        echo '</ul>';
	        echo '</div>' . "\n";
			echo '<button class="_button" id="_save"><i class="icon fa-save"></i><span class="r-save-text">' . esc_html__( 'Save Settings', 'zona' ) . '</span></button>';
			echo '</div>';
			...
			echo '<div class="box-row clearfix">
				<div class="box-row-input">
					<div class="box-tc box-tc-label">
						<label for="color">' . esc_html__( 'Export Data', 'zona' ) . '</label>
					</div>
					<div class="box-tc box-tc-input">
						<textarea name="export" style="height:200px;overflow:auto">' . wp_kses_post( $export ) . '</textarea>
					</div>
				</div>
		...
		private function import( $value ) {	
			...
			echo '<div class="box-row clearfix">
					<div class="box-row-input">
						<div class="box-tc box-tc-label">
							<label for="color">' . esc_html__( 'Import Data', 'zona' ) . '</label>
						</div>
						<div class="box-tc box-tc-input">
							...
							<div class="clear"></div>
							<button class="_button data-import"><i class="fa fa-upload icon"></i>' . esc_html__( 'Import data', 'zona' ) . '</button>
							<p class="help-box">' . esc_html__( 'Click on "Import data" button and paste in the box above previously exported data, and press the save button.', 'zona' ) . '</p>
						</div>
					</div>
					<div class="box-row-line"></div>
				</div>';

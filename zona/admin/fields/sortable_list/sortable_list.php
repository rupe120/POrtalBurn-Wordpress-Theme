        ...
        */
		public function render() {
			...
			if ( isset(self::$_option['button_text']) && self::$_option['button_text'] != '' ) {
			    ...
			    $button_text = esc_html__( 'Add New Item', 'zona' );
			}
			...
			foreach ( self::$_option['id'] as $count => $item ) {
			    echo '<label>' . esc_html( $item['label'] ) . '</label>';
				echo '<input type="text" value="" name="' . esc_attr( $item['id'] ) . '[]"/>';
			}
			echo '</div>';
			echo '</li>';
			echo '</ul>';
				...
				foreach ( self::$_saved_options[self::$_option['array_name']] as $items ) {
					...
					foreach ( self::$_option['id'] as $count => $item ) {
						...
						if ( isset( $items[$item['name']] ) ) {
						    echo '<input type="text" class="input" value="' . esc_html($items[$item['name']]) . '" name="' . esc_attr($item['id']) . '[]"/>';
						} else {
						    echo '<input type="text" class="input" value="" name="' . esc_attr($item['id']) . '[]"/>';
						}
					}
					echo '</div>';
					echo '</li>';
